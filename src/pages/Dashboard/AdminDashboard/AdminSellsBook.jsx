...
const MyTextInput = ({ label, ...props }) => {
  ...
  return (
    ...
    <div className="flex flex-col">
      <label className='text-md text-poppins text-md ms-1' htmlFor={props.id || props.name}>{label}</label>
      <input className="text-input border rounded-sm border-gray-600 ps-4 mt-3 h-10 w-[300px] lg:me-3" {...field} {...props} />
      ...
      <select className="text-input border rounded-sm border-gray-600 ps-4 mt-3 h-10 w-[300px] lg:me-3" onChange={handleCategory}  >
        {allCategory.map(item=><option key={item.category} value={item.category}>{item.category}</option> ) }
          ...
          <div className='flex flex-col md:ms-4'>
                ...
                <input type="file" onChange={(e)=>{setSelectedImages(e.target.files[0])}} placeholder="Image" name='image' className="text-input border rounded-sm border-gray-600 ps-4 mt-3 h-10 w-[300px] lg:me-3"/>
            </div>
          </div>
          ...
          <div className='flex flex-col'>
            <label className='text-md text-poppins text-md ms-1 mt-3' htmlFor="description" >Description</label>
            <textarea name='description' id="description" type="description" {...formik.getFieldProps('description')} className="w-full mt-5 border border-black rounded-md" rows={10} />
          </div>
        
          <button className='btn btn-primary w-60 mt-5 justify-center mx-auto rounded-md' type="submit">Submit</button>
        </Form>)}
      </Formik>
       </div> 
    </div>
